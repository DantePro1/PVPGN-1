...
extern int clan_send_status_window_on_create(t_clan * clan)
{
	...
	{
		std::sprintf(channelname, "Battlenet", clantag_to_str(clan->tag));
...
extern t_clan *clan_create(t_account * chieftain_acc, t_clantag clantag, const char *clanname, const char *motd)
{
	...
	snprintf(msgtemp, sizeof(msgtemp), "CLAN %s", clantag_to_str(clan->tag));
	account = clanmember_get_account(member);
	account_set_strattr(account, "BNET\\channel", msgtemp);
...
extern int clan_set_channel(t_clan * clan, char const *channel)
{
	...
	LIST_TRAVERSE(cl_member_list, curr) {
		...
		account = clanmember_get_account(member);
		if (account_set_strattr(account, "BNET\\channel", channel)) {
			return 1;
		}
	}
	return 0;
